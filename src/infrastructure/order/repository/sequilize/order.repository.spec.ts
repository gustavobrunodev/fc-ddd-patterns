...
describe("Order repository test", () => {
  ...
  it("should update an order", async () => {
    ...
    const product2 = new Product("1234", "Product 2", 50);
    ...
    const orderToUpdate = new Order(order.id, customer.id, [
      orderItem,
      orderItem2,
    ]);
  ...
  it("should throw an error when order is not found", async () => {
    const orderRepository = new OrderRepository();
    await expect(orderRepository.find("non_existing_id")).rejects.toThrow(
      "Order not found"
    );
  });
});
